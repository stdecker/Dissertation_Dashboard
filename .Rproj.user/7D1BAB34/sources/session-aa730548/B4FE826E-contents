...
# for(y in unique(adp_mm$Tissue)){
#   adp_kinetics <- ggplot(data = subset(adp_mm, Tissue %in% paste(y)), aes(x = Concentration, y = Rate, fill = Smoke)) +
#     theme_prism() +
#     #stat_summary(geom = "errorbar", fun.data = "mean_se", size = 1, width = 100) +
#     geom_errorbar(data = subset(adp_errors, Tissue %in% paste(y)), aes(x = Concentration, y = Rate_mean, ymin = min, ymax = max), inherit.aes = F, size = 1, width = 75, color = "black") +
#     coord_cartesian(ylim = c(0, 80), xlim = c(-100, 5500), expand = F) +
#     scale_y_continuous(expand = c(0,0), breaks = seq(0,80,10)) +
#     labs(x = "[ADP] (µM)", y = expression(bold(Respiration~Rate~(pmol[O[2]]/sec/mg[wt])))) +
#     geom_line(data = subset(adp_mml, Tissue %in% paste(y)), aes(x = S, y = v, linetype = Smoke), size = 1) +
#     scale_linetype_manual(breaks = c("Control", "Smoke"), values = c(2, 1)) +
#     scale_fill_manual(values = c("white", "grey25")) +
#     geom_point(aes(x = Concentration, y = Rate, fill = Smoke), stat = "summary", fun.y = "mean", size = 3.5, pch = 21, color = "black")
#   
#   assign(paste(y, "adp_kinetics", sep = "_"), adp_kinetics)
# }
...
adp_kinetics <- ggplot(data = adp_mm, aes(x = Concentration, y = Rate, color = Condition)) +
  theme_prism() +
  #stat_summary(geom = "errorbar", fun.data = "mean_se", size = 1, width = 100) +
  geom_errorbar(data = adp_errors, aes(x = Concentration, y = Rate_mean, ymin = min, ymax = max, color = Condition), inherit.aes = F, size = 1, width = 75, show.legend = F) +
  coord_cartesian(ylim = c(0, 80), xlim = c(-100, 5500), expand = F) +
  scale_y_continuous(expand = c(0,0), breaks = seq(0,80,10)) +
...
cat_errors <- cat_mm[-c(1,3,5,6,7,8,9)] |> 
  group_by(Tissue, Smoke, Concentration) |> 
  summarise_each(funs = funs(mean, se=sd(.)/sqrt(n())))

cat_errors$min <- cat_errors$Rate_mean + (cat_errors$Smoke == "Smoke")*cat_errors$Rate_se
cat_errors$max <- cat_errors$Rate_mean + (cat_errors$Smoke == "Control")*cat_errors$Rate_se

# for(y in unique(cat_mm$Tissue)){
#   cat_kinetics <- ggplot(data = subset(cat_mm, Tissue %in% paste(y)), aes(x = Concentration, y = Rate, fill = Smoke)) +
#     theme_prism() +
#     #stat_summary(geom = "errorbar", fun.data = "mean_se", size = 1) +
#     geom_errorbar(data = subset(cat_errors, Tissue %in% paste(y)), aes(x = Concentration, y = Rate_mean, ymin = min, ymax = max), inherit.aes = F, size = 1) +
#     coord_cartesian(ylim = c(0, 70), xlim = c(-0.25, 5.25), expand = F) +
#     scale_y_continuous(expand = c(0, 0), breaks = seq(0, 70, 10)) +
#     labs(x = "[CAT] (µM)", y = expression(bold(Respiration~Rate~(pmol[O[2]]/sec/mg[wt])))) +
#     geom_line(data = subset(cat_mml, Tissue %in% paste(y)), aes(x = S, y = v, fill = Smoke, linetype = Smoke), size = 1, color = "black") +
#     scale_linetype_manual(breaks = c("Control", "Smoke"), values = c(2, 1)) +
#     scale_fill_manual(values = c("white", "grey25")) +
#     geom_point(aes(x = Concentration, y = Rate, fill = Smoke), stat = "summary", fun.y = "mean", size = 3.5, pch = 21, color = "black")
#   
#   assign(paste(y, "cat_kinetics", sep = "_"), cat_kinetics)
# }

cat_mml$Condition <- paste(cat_mml$Tissue, cat_mml$Smoke)
...
cat_kinetics <- ggplot(data = cat_mm, aes(x = Concentration, y = Rate, color = Condition)) +
  theme_prism() +
  #stat_summary(geom = "errorbar", fun.data = "mean_se", size = 1, width = 100) +
  geom_errorbar(data = cat_errors, aes(x = Concentration, y = Rate_mean, ymin = min, ymax = max, color = Condition), inherit.aes = F, size = 1, fill = "white", show.legend = F) +
...
TC_anova <- anova(art(data = cat_data[-c(34),], Thermodynamic_Coupling ~ Smoke * Tissue))
...
SpecificRate_anova <- anova(art(data = cat_data[-c(25,27),], SpecificRate ~ Smoke * Tissue))
...
RotFCCP_anova <- anova(art(data = cat_data[-c(9),], CIIPercent ~ Smoke * Tissue))
...
all_adp <- ggarrange(adp_km, adp_vmax, common.legend = T, nrow = 1, legend = "bottom", labels = "AUTO")


all_cat <- ggarrange(cat_vmax, cat_km, cat_imax, cat_percent, common.legend = T, legend = "bottom", labels = "AUTO")


all_cat2 <- ggarrange(cat_km, cat_imax, cat_percent, common.legend = T, nrow = 1, legend = "bottom", labels = "AUTO")


calculated_vars <- ggarrange(cat_RCR, cat_Thermodynamic_Coupling, cat_RotFCCP, common.legend = T, nrow = 1, legend = "bottom", labels = "AUTO")
...
DE
'
adp_combined <- adp_kinetics + adp_km + adp_vmax & theme(legend.position = "bottom")

all_adp2 <- adp_combined + plot_layout(design = layout_adp, guides = 'collect') + plot_annotation(tag_levels = 'A')


cat_combined <- cat_kinetics + cat_vmax + cat_km + cat_imax + cat_percent & theme(legend.position = "bottom")

all_cat3 <- cat_combined + plot_layout(design = layout_cat, guides = 'collect') + plot_annotation(tag_levels = 'A')



## Individual Plots

for(x in unique(cat_long$State)){
  indiv_plot <- ggplot(data = subset(cat_long[-c(69, 85, 578, 595, 597, 612, 614, 629),], State %in% paste(x)), aes(x = State, y = Rate, color = Condition)) +
    stat_summary(geom = "bar", position = position_dodge(0.99), size = 1, fill = "white") +
    #geom_point(position = position_dodge(0.9), size = 2, pch = 21, stroke = 1.5, show.legend = FALSE) +
    theme_prism() +
    scale_color_manual(values = c("blue", "darkblue", "red", "darkred")) +
    ...
    #                                                 , 'Rot', 'AmA & Omy')), aes(x = State, y = Rate, fill = Condition), 
    #            position = position_dodge(0.99), size = 2, pch = 21, stroke = 1.5, show.legend = FALSE) +
    geom_beeswarm(data = subset(cat_long[-c(69, 85, 578, 595, 597, 612, 614, 629),], State %in% paste(x)), 
                  aes(x = State, y = Rate, color = Condition), 
                  dodge.width = 0.99, size = 2, pch = 21, stroke = 1.5, show.legend = FALSE, groupOnX = T) +
    scale_fill_manual(values = c("white", "white", "white", "white"))
  
  if(paste(x) == "GM"){indiv_plot <- indiv_plot + annotate('text', x = .51, y = 198, label = 'Smoke: p = 0.150', hjust = 0, fontface = 2) +
                                                                annotate('text', x = .51, y = 191, label = 'Tissue: p < 0.001', hjust = 0, fontface = 2) +
                                                                annotate('text', x = .51, y = 184, label = 'Interaction: p = 0.175', hjust = 0, fontface = 2)
  }
  
  if(paste(x) == "GMD 5000"){indiv_plot <- indiv_plot + annotate('text', x = .51, y = 198, label = 'Smoke: p < 0.001', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 191, label = 'Tissue: p = 0.001', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 184, label = 'Interaction: p = 0.604', hjust = 0, fontface = 2) +
    geom_bracket(xmin = c(.625), xmax = c(.885), y.position = 70, inherit.aes = F, label = "p = 0.014", size = 1, fontface = 2, tip.length = 0.01) +
    geom_bracket(xmin = c(1.115), xmax = c(1.35), y.position = 125, inherit.aes = F, label = "p = 0.038", size = 1, fontface = 2, tip.length = 0.01) +
    scale_x_discrete(labels = c(expression(bold(GMD['5000 μM'])))) 
    
  }
  
  if(paste(x) == "GMDS"){indiv_plot <- indiv_plot + annotate('text', x = .51, y = 198, label = 'Smoke: p < 0.001', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 191, label = 'Tissue: p < 0.001', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 184, label = 'Interaction: p = 0.604', hjust = 0, fontface = 2) +
    geom_bracket(xmin = c(.625), xmax = c(.885), y.position = 75, inherit.aes = F, label = "p = 0.052", size = 1, fontface = 2, tip.length = 0.01) +
    geom_bracket(xmin = c(1.115), xmax = c(1.345), y.position = 160, inherit.aes = F, label = "p = 0.080", size = 1, fontface = 2, tip.length = 0.01) 
  }
  
  if(paste(x) == "CAT 5.0"){indiv_plot <- indiv_plot + annotate('text', x = .51, y = 198, label = 'Smoke: p = 1.000', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 191, label = 'Tissue: p < 0.001', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 184, label = 'Interaction: p = 0.814', hjust = 0, fontface = 2) +
    scale_x_discrete(labels = c(expression(bold(CAT['5.0 μM'])))) 
    
  }
  
  if(paste(x) == "FCCP Peak"){indiv_plot <- indiv_plot + annotate('text', x = .51, y = 198, label = 'Smoke: p = 0.004', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 191, label = 'Tissue: p < 0.001', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 184, label = 'Interaction: p = 0.857', hjust = 0, fontface = 2) +
    geom_bracket(xmin = c(.625), xmax = c(.885), y.position = 95, inherit.aes = F, label = "p = 0.122", size = 1, fontface = 2, tip.length = 0.01) +
    geom_bracket(xmin = c(1.115), xmax = c(1.345), y.position = 165, inherit.aes = F, label = "p = 0.101", size = 1, fontface = 2, tip.length = 0.01) +
    scale_x_discrete(labels = c(expression(bold(FCCP[Peak]))))
    
  }
  
  if(paste(x) == "Rot"){indiv_plot <- indiv_plot + annotate('text', x = .51, y = 198, label = 'Smoke: p = 0.022', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 191, label = 'Tissue: p < 0.001', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 184, label = 'Interaction: p = 0.468', hjust = 0, fontface = 2)
  }
  
  if(paste(x) == "AmA & Omy"){indiv_plot <- indiv_plot + annotate('text', x = .51, y = 198, label = 'Smoke: p = 0.004', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 198, label = 'Smoke: p = 0.072', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 191, label = 'Tissue: p < 0.001', hjust = 0, fontface = 2) +
    annotate('text', x = .51, y = 184, label = 'Interaction: p = 0.535', hjust = 0, fontface = 2)
  }
...
resp_combined_plot <- mainplot_all + cat_RotFCCP + cat_GMDSCAT + cat_GMDSFCCP + cat_Thermodynamic_Coupling & theme(legend.position = "bottom")

all_resp2 <- resp_combined_plot + plot_layout(design = layout_resp, guides = 'collect') + plot_annotation(tag_levels = 'A') 

diffs <- (FCCP_diff + CAT_diff) + plot_layout(guides = 'collect') + plot_annotation(tag_levels = 'A') 

# IALS_all <- ggplot(data = subset(cat_long[-c(69, 85, 578, 595, 597, 612, 614, 629),], State %in% c('GM', 'GMD 5000', 'GMDS', 'Rot')), aes(x = State, y = Rate, fill = Condition)) +
...
#   theme(axis.title.x = element_blank(),
#         legend.position = "bottom") +
#   ggtitle("Mitochondrial Respiration") +
#   geom_vline(xintercept = c(1.5, 2.5, 3.5), linetype = "longdash", color = "grey50", size = .6) + 
#   # geom_point(data = subset(cat_long, State %in% c('GM', 'GMD 5000', 'GMDS', "CAT 5.0", 'FCCP Peak'
#   #                                                 , 'Rot', 'AmA & Omy')), aes(x = State, y = Rate, fill = Condition), 
#   #            position = position_dodge(0.99), size = 2, pch = 21, stroke = 1.5, show.legend = FALSE) +
#   geom_beeswarm(data = subset(cat_long[-c(69, 85, 578, 595, 597, 612, 614, 629),], State %in% c('GM', 'GMD 5000', 'GMDS', 'Rot')), 
#                 aes(x = State, y = Rate, fill = Condition), 
#                 dodge.width = 0.99, size = 2, pch = 21, stroke = 1.5, show.legend = FALSE, groupOnX = T) +
#   scale_fill_manual(values = c("white", "white", "white", "white")) +
#   scale_x_discrete(labels = c(expression(bold(GM)), expression(bold(GMD['5000 μM'])), expression(bold(GMDS)), expression(bold(Rot)))) +
#   #Basal
#   # annotate('text', x = 0.45, y = 198, label = 'Smoke: p = 0.408', hjust = 0, fontface = 2) +
#   # annotate('text', x = 0.45, y = 191, label = 'Tissue: p = 0.283', hjust = 0, fontface = 2) +
#   # annotate('text', x = 0.45, y = 184, label = 'Interaction: p = 0.829', hjust = 0, fontface = 2) +
...
#   # geom_bracket(xmin = c(4.885), xmax = c(5.345), y.position = 172, inherit.aes = F, label = "p = 0.015", size = 1, fontface = 2, tip.length = 0.01) +
#   #Rot
#   annotate('text', x = 3.51, y = 198, label = 'Smoke: p = 0.022', hjust = 0, fontface = 2) +
#   annotate('text', x = 3.51, y = 191, label = 'Tissue: p < 0.001', hjust = 0, fontface = 2) +
#   annotate('text', x = 3.51, y = 184, label = 'Interaction: p = 0.468', hjust = 0, fontface = 2)# +
# # geom_bracket(xmin = c(5.665), xmax = c(6.115), y.position = 100, inherit.aes = F, label = "p = 0.002", size = 1, fontface = 2, tip.length = 0.01) +
# # geom_bracket(xmin = c(5.885), xmax = c(6.345), y.position = 110, inherit.aes = F, label = "p = 0.015", size = 1, fontface = 2, tip.length = 0.01) +
# #Ama & Omy
# # annotate('text', x = 6.51, y = 198, label = 'Smoke: p = 0.072', hjust = 0, fontface = 2) +
...
# 
# 
# cat_RotFCCPbw <- ggplot(data = cat_data[-c(9),], aes(x = Condition, y = CIIPercent, fill = Condition)) +
#   stat_summary(geom = "bar", position = position_dodge(0.99), size = 1, color = "black") +
#   theme_prism() +
#   scale_fill_manual(values = c("white", "grey33", "grey66", "black")) +
#   coord_cartesian(ylim = c(0, 1.1), clip = "off") +
#   scale_y_continuous(expand = c(0,0), breaks = seq(0, 1.1, .1), labels = label_percent()) +
#   labs(y=expression(bold(Complex~II~Contribution~('Rot/FCCP'['Peak']))), x = "") +
#   ggtitle("Complex II Contribution") +
#   # geom_point(data = individ_cat_kinetics, aes(x = Condition, y = CIIPercent, fill =  Condition), position = position_dodge(0.99), size = 2, pch = 21, stroke = 1.5, show.legend = FALSE) +
#   geom_beeswarm(data = cat_data[-c(9),], aes(x = Condition, y = CIIPercent, fill =  Condition), position = position_dodge(0.99), size = 2, pch = 21, stroke = 1.5, show.legend = FALSE, cex = 3) +
#   scale_fill_manual(values = c("white", "white", "white", "white")) +
#   theme(axis.text.x = element_blank(),
#         axis.ticks.x = element_line(color = NA),
#         legend.position = "bottom") +
#   annotate('text', x = .55, y = 1.089, label = 'Smoke: p = 0.015', hjust = 0, fontface = 2) +
#   annotate('text', x = .55, y = 1.049, label = 'Tissue: p = 0.466', hjust = 0, fontface = 2) +
#   annotate('text', x = .55, y = 1.009, label = 'Interaction: p = 0.048', hjust = 0, fontface = 2) +
#   geom_bracket(xmin = "Gastrocnemius Control", xmax = "Gastrocnemius Smoke", y.position = 0.88, inherit.aes = F, label = "p = 0.006", size = 1, fontface = 2, tip.length = 0.02)
# 
# 
# poster1 <- ggarrange(IALS_all, cat_RotFCCPbw, common.legend = T, legend = "bottom", widths = c(0.65,.25), heights = c(1, 0.8))
# 
# ggsave("IALS_all.png", IALS_all, width = 12)
# 
# ggsave("poster1.png", poster1, width = 13)
# 
# poster2 <- ggarrange(cat_RCR, cat_Thermodynamic_Coupling, common.legend = T, legend = "bottom", nrow = 1)
# 
# ggsave("poster2.png", poster2, width = 12)
