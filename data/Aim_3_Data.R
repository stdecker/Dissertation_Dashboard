...
# Import data ----
 cat_data <- read_excel('Analysis Files\\Project 4\\Analysis File4.xlsx', sheet = "Final Data")
 ...
 cat_data$Thermodynamic_Coupling <- sqrt(1-(cat_data$`AmA & Omy`/cat_data$`FCCP Peak`))
...
cat_imax <- ggplot(data = individ_cat_kinetics, aes(x = Condition, y = Imax, color = Condition)) +
  ...
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(), 
        axis.ticks.x = element_blank()) +
  annotate('text', x = 0.55, y = 176, label = 'Smoke: p = 0.056', hjust = 0, fontface = 2) +
  ...
  geom_bracket(xmin = "Gastrocnemius Control", xmax = "Soleus Control", y.position = 132, inherit.aes = F, label = "p = 0.001", size = 1, fontface = 2, tip.length = 0.02) +
  geom_bracket(xmin = "Gastrocnemius Smoke", xmax = "Soleus Smoke", y.position = 147, inherit.aes = F, label = "p = 0.003", size = 1, fontface = 2, tip.length = 0.02) 
...
cat_km <- ggplot(data = individ_cat_kinetics[-c(36, 7),], aes(x = Condition, y = IC50, color = Condition)) +
  ...
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(), 
        axis.ticks.x = element_blank()) +
  annotate('text', x = 0.55, y = 9.8, label = 'Smoke: p = 0.916', hjust = 0, fontface = 2) +
  annotate('text', x = 0.55, y = 9.3, label = 'Tissue: p = 0.593', hjust = 0, fontface = 2) +
  annotate('text', x = 0.55, y = 8.8, label = 'Interaction: p = 0.796', hjust = 0, fontface = 2)
...
cat_percent <- ggplot(data = individ_cat_kinetics[-c(7, 24),], aes(x = Condition, y = Inhibition, color = Condition)) +
  ...
  annotate('text', x = 0.55, y = -1.2, label = 'Tissue: p = 0.165', hjust = 0, fontface = 2) +
  annotate('text', x = 0.55, y = -1.26, label = 'Interaction: p = 0.057', hjust = 0, fontface = 2) +
...
# 
# for(y in unique(cat_long$State)){
#   anova_states <- anova(art(Rate ~ Smoke * Tissue, data = subset(cat_long[-c(69, 85, 578, 595, 597, 612, 614, 629),], State %in% paste(y))))
...
# subset(resp_descriptives, State %in% "GM")
# 
# outliers <- data.frame()
# 
...
etc_analysis <- pivot_wider(cat_long, id_cols = Subject, values_from = Rate, names_from = c(Condition, State))
...
FCCP_diff <- ggplot(data = subset(etc_analysis_long, State %in% "difference"), aes(x = Condition, y = value, fill = Condition)) +
  ...
  geom_beeswarm(data = subset(etc_analysis_long, State %in% "difference"), aes(x = Condition, y = value, fill = Condition), position = position_dodge(0.99), size = 2, pch = 21, stroke = 1.5, show.legend = FALSE, cex = 3) +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(), 
        axis.ticks.x = element_blank(),
...
CAT_diff <- ggplot(data = subset(etc_analysis_long[-c(466),], State %in% "percentage"), aes(x = Condition, y = value, fill = Condition)) +
  stat_summary(geom = "bar", position = position_dodge(0.99), size = 1, color = "black") +
  theme_prism() +
  scale_fill_manual(values = c("white", "grey50")) +
  ...
  #ggtitle(expression(bold(Delta~FCCP[Peak]~"(Smoke"-"Control)"))) +
  # geom_point(data = individ_cat_kinetics, aes(x = Condition, y = CIIPercent, fill =  Condition), position = position_dodge(0.99), size = 2, pch = 21, stroke = 1.5, show.legend = FALSE) +
  geom_beeswarm(data = subset(etc_analysis_long[-c(466),], State %in% "percentage"), aes(x = Condition, y = value, fill = Condition), position = position_dodge(0.99), size = 2, pch = 21, stroke = 1.5, show.legend = FALSE, cex = 3) +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(), 
        axis.ticks.x = element_blank(),
        axis.line.x = element_blank()) +
...
# annotate('text', x = 3.55, y = 3.32, label = 'Tissue: p = 0.006', hjust = 0, fontface = 2) +
# annotate('text', x = 3.55, y = 3.19, label = 'Interaction: p = 0.006', hjust = 0, fontface = 2) +
 geom_bracket(xmin = "Gastrocnemius", xmax = "Soleus", y.position = -.66, inherit.aes = F, label = "p = 0.024", size = 1, fontface = 2, tip.length = -0.02, vjust = 2.5)# +
# geom_bracket(xmin = "Soleus Control", xmax = "Soleus Smoke", y.position = 2.5, inherit.aes = F, label = "p = 0.075", size = 1, fontface = 2, tip.length = 0.02)

identify_outliers(subset(subset(etc_analysis_long, Condition %in% "Gastrocnemius"), State %in% "percentage"), value)
